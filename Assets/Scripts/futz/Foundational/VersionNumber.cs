...
{
/// Note: doesn't set Unity build version
[CreateAssetMenu(menuName = nameof(VersionNumber))]
public class VersionNumber : ScriptableObject
{
	...
	[UnityEditor.MenuItem("CONTEXT/VersionNumber/Set Version")]
	static void SetVersion(UnityEditor.MenuCommand command)
		=> ((VersionNumber)command.context).Set("set by menu");
	...
	{
		var assetGuids = UnityEditor.AssetDatabase.FindAssets($"t:{nameof(VersionNumber)}");

		foreach (var guid in assetGuids) {
			var path = UnityEditor.AssetDatabase.GUIDToAssetPath(guid);
			var asset = UnityEditor.AssetDatabase.LoadAssetAtPath(path, typeof(VersionNumber))
				as VersionNumber;
